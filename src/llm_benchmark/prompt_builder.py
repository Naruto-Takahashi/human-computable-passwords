...
class TextPromptBuilder(PromptBuilder):
    ...
    _SYSTEM_INSTRUCTION: str = (
        "あなたは与えられた入出力ペアを観察し，"
        "背後に隠れたルールを推論して新しい入力に対する出力を予測する専門家です．\n\n"
        "以下の問題では，入力は14個の整数からなる数列（X0〜X13），"
        "出力は0から9の整数1桁（Z）です．\n"
        "これらのペアを観察し，その変換ルールを推論してください．"
        "秘密の鍵テーブルや参照アルゴリズムが存在する可能性があります．\n\n"
    )

    # Few-shot 例題セクションのヘッダー
    _EXAMPLES_HEADER: str = "【観察データ（入出力ペア）】\n"

    # テスト問題セクション
    _QUESTION_HEADER: str = "\n【予測課題】\n"
    _ANSWER_INSTRUCTION: str = (
        "\n上記の観察データのパターンに基づいて，以下の入力に対する出力 Z を予測してください．\n"
        "推論過程を記述した後，必ず最後の行に以下の形式で回答してください：\n"
        "Answer: <0〜9の整数1桁>\n\n"
        "入力: {challenge}\n"
        "Z = "
    )
