...
> コードがいまの形になった経緯。時期ごとの記録
>
> 📚 [ドキュメント索引](README.md) ／ 関連: [結果の置き場](../results/README.md) ・ [研究計画](plan.md)

コードベースを大きく変えたときの記録．**なぜいまの形になっているか**を残す。

| 時期 | 内容 |
|---|---|
| [2026-07-12](#2026-07-12-研究計画-v2-への移行) | 研究計画 v2 への移行に伴う全面リファクタリング |
| [2026-09-12〜13](#2026-09-1213-測定系の欠陥修正と見通しの整理) | 測定系の欠陥修正と，コード・ドキュメントの見通しの整理 |

---

## 2026-09-12〜13: 測定系の欠陥修正と見通しの整理

実験7 で「測っていたのは関数の難しさではなく，5エポックという予算の中で学習が
離陸したかどうかだった」と判明したことを受け，測定系とその周辺を作り直した。
経緯は [measurement_audit.md](measurement_audit.md)，実験の対応は
[experiment_index.md](experiment_index.md)。

### 正しさに関わる修正

1. **評価結果の保存先に評価件数が入っていなかった**
   `make_run_dir` の条件が `n{n_shot}_stage{S}_k{K}` で，ファインチューニング評価
   （`n_shot=0` 固定）では件数が違っても同じ場所に書き込まれていた。50件の結果を
   500件で上書きする事故が 2026-08 に実際に発生しており，さらに `is_run_completed()`
   が `metrics.json` の有無しか見ないため，50件で評価済みの条件が500件のスイープで
   スキップされる状態だった。条件を `n{N}_t{T}_stage{S}_k{K}` に変更し，既存57件を移行。

2. **学習シード・学習率スケジュールを指定できなかった**
   HuggingFace の既定値（`seed=42`, `linear`）が暗黙に効いているだけで，振る手段も
   記録もなかった。`--seed` / `--lr_scheduler` / `--warmup_ratio` を追加し，
   `train_metadata.json` に残るようにした。難易度を「離陸に要するステップ数」で
   測るには，予算の宣言が測定値に混ざらない定数学習率が要る。

3. **アルゴリズムの文面が非退行検査で守られていなかった**
   学習済みアダプタは `rule_text` の文字列で学習しているため，表現を1文字でも変えると
   過去の結果と比較できなくなる。`tests/check_algorithm_texts.py` を追加して
   `make test` に組み込み，35アルゴリズムの文面・計算結果・解説を基準と照合する。

### 構造の整理

- **評価結果をアルゴリズム先頭に**
  `llm_eval/{モデル}/{アルゴリズム}/…` はモデル名が学習 run ごとに変わるため，
  同じアルゴリズムの結果が60個のディレクトリに散っていた。
  `llm_eval/{アルゴリズム}/{タスク}/{条件}/{シード}/{モデル}/` に変更（60→26）。
...
- **バッチの共通化**
  ...
  集約し，実行済みのものは `done/` へ退避。`HCP_DRY_RUN=1` で GPU を掴まずに疎通確認できる。
- **運用コマンドの追加**
  `make status`（走行中バッチと進捗）・`make inventory`（学習 run の棚卸し）・
  `make algorithms`（アルゴリズム一覧）・`make migrate-eval-paths`。
  それまで `grep -c '✓'` を手打ちしていた作業を置き換えた。

### ドキュメントの整理

- 索引（`docs/README.md`）を新設し，目的別に引けるようにした
- 週次報告の「前提知識」節が報告ごとに手で複製されていた（0819 と 0823 で15節すべて重複）
  ため，[hcp_background.md](hcp_background.md) を正本にした
- 「段階」が3つの意味（情報開示の Stage 0〜3／実験の通し番号／7月の別系統）で使われ，
...
## 2026-07-12: 研究計画 v2 への移行

> [!WARNING]
> **以下は当時の記録である．** ディレクトリ構造やコマンドは 2026-09 の整理で
> 変わっている箇所がある（上の節を参照）。現行の手順は `make help` と
> [README.md](../README.md) にある。

研究計画 v2（現 `plan.md`）への移行に伴い，コードベースの監査と全面リファクタリングを実施した．本ノートは (1) 監査で見つかった研究妥当性に関わる問題，(2) 新アーキテクチャ，(3) 運用手順をまとめる．
...
> [!WARNING]
> **当時の構造．** 2026-09 に評価件数 `t{T}` を条件に加え，アルゴリズムを先頭へ移した
> （上の「2026-09-12〜13」節）。現行の構造は [../results/README.md](../results/README.md)。
...
| | |
|---|---|
| [結果の置き場](../results/README.md) | 現行のディレクトリ構造 |
