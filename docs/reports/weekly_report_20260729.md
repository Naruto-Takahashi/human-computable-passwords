...
**チャレンジの長さ（常に14個，X0〜X13）と，鍵（SGM_TABLE）のマス数は無関係の別パラメータであり，一致させる必要はない。** チャレンジの各値は「鍵の何番目のマスを見るか」を指定する**添字（インデックス）**であって，計算に使う値そのものではない。以下，実際に使用した鍵とチャレンジの例で図解する（鍵のマス数が26のケース）。

```
■ 鍵（SGM_TABLE）: マス数は実験ごとに設定するパラメータ（この例では26。
  ４桁実験では４，１０，... のように変える。チャレンジの長さ14とは無関係）
...
■ チャレンジ（X0〜X13）: 要素数は常に14個で固定。各値は「鍵の添字」であり，
  とりうる範囲（この例では0〜25）は鍵のマス数によって変わるが，要素数14は変わらない

  位置 :  X0  X1  X2  X3  X4  X5  X6  X7  X8  X9 X10 X11 X12 X13
  値  :  17  10  19   5   4  14  16  17  10  18  14   1  15  25
              　　　　　　　　　　　　　　　　　　　　↑　 ↑　 ↑　 ↑
                                                   この4つが func_22 の計算で使われる

  読み方の例：「X10 = 14」は「鍵の14番目のマスを見よ」という意味の添字。
  実際に計算へ使われる値は，鍵を引いた後の SGM_TABLE[14] = 9 である。
```

`func_22`（本命関数）は，このチャレンジ・鍵から次の手順でレスポンスZを計算する：

```
ステップ1: X10, X11 の添字で鍵を引く
    X10=14 → SGM_TABLE[14] = 9
    X11= 1 → SGM_TABLE[ 1] = 6

ステップ2: その2値から「次にどこを見るか」を計算する（★動的参照）
    j = (9 + 6) mod 10 = 5

ステップ3: j=5 なので，チャレンジの5番目の添字を新たに読みに行く
    X5 = 14 → SGM_TABLE[14] = 9   ← 参照先がここで初めて決まる（jは入力ごとに変わる）

ステップ4: X12, X13 の添字でも鍵を引く
    X12=15 → SGM_TABLE[15] = 7
    X13=25 → SGM_TABLE[25] = 8

ステップ5: 4つの値を合計し1桁にする
    Z = (9 + 7 + 8) mod 10 = 4   ← これが正解レスポンス
```
