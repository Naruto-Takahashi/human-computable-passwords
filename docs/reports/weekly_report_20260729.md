...
```mermaid
flowchart LR
    subgraph CH["チャレンジ（要素数は常に14個で固定，X0〜X13）"]
        direction LR
        X10["X10 = 14"]
        X11["X11 = 1"]
        X5["X5 = 14"]
    ...
    subgraph KEY["鍵 SGM_TABLE（マス数は実験ごとに設定するパラメータ。この例は26マス）"]
        direction LR
        K1["添字1 → 値6"]
        K14["添字14 → 値9"]
        K15["添字15 → 値7"]
...
チャレンジの値そのもの（例: `X10 = 14`）は計算に使われず，「鍵の14番目のマスを見よ」という指示にすぎない。実際に計算へ使われるのは，鍵を引いた後の値（`SGM_TABLE[14] = 9`）である。

`func_22`（本命関数）は，このチャレンジ・鍵から次の手順でレスポンスZを計算する：

```mermaid
flowchart TB
    A["X10=14, X11=1"] -->|"鍵を引く"| B["9, 6"]
    B -->|"j = (9+6) mod 10"| C["j = 5"]
    C -->|"チャレンジの5番目（X5=14）を新たに参照"| D["X5 = 14"]
    D -->|"鍵を引く（★動的参照：参照先jは入力ごとに変わる）"| E["SGM_TABLE[14] = 9"]
    F["X12=15, X13=25"] -->|"鍵を引く"| G["7, 8"]
    E --> H["Z = (9+7+8) mod 10 = 4"]
    G --> H
```
