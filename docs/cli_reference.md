...
- **鍵が小さい課題では既出／未出に分ける**．`make seen-unseen RUN=<評価の run>` で，
  答えを決める位置の組が学習で出ていたかを判定し，正解率を分けて出す．
  たとえば `narrowptr_k10_m1` は答えを決める位置が3つしかなく，評価500件のうち
  63%が既出だった（未出だけでも 96.2% なので丸暗記ではない，と初めて言える）
