...
| 条件 | 動的参照 | 位置数 | 足す個数 | 組合せ | 正解率 | **未出** | 離陸 |
|---|---|---|---|---|---|---|---|
| `narrowptr_k4_m1` | なし | 3 | 3 | 64 | 100.0% | —（既出のみ） | ep3 |
| `narrowptr_k4_m2` | **あり** | 6 | 3 | 4,096 | **100.0%** | **399/399 = 100.0%** | ep12 |
| `table_add6_k4` | なし | 6 | **6** | 4,096 | 10.6% | 47/401 = 11.7% | **せず** |

- **$`m=2`$ は丸暗記ではない。**未出399件すべて正解。検証損失は
  0.1872 → 0.1247 → … → ep12 で 0.0000 と**単調に降下**しており，
  12エポックかけて規則を獲得している。
...
$`m=2`$ は位置を6個読むが**足すのは3個**，`table_add6` は**6個足す**。
位置数・空間の大きさを揃えても結果が割れたので，残る違いは**加算の項数**である。

一方 `narrowptr_k4_m2`（100%）と `narrowptr_k10_m2`（9.2%）は項数3で同じだが
鍵サイズが違う（組合せ 4,096 対 1,000,000）。**空間の大きさも効いている。**

つまり壁は少なくとも2つの軸からなり，**動的参照はその軸に含まれない**。
