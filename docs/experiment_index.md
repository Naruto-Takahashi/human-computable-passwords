...
| 3 | **正解率は連続量ではない** | 全56runで 44% と 96% の間が空白。最小検証損失 0.02 で**例外なく**分かれる |
| 4 | **正解率は「予算内に収束したか」の二値読み** | 20エポックで回した8本中2本は収束が5エポック目より後 |
...
| | 主張 | 足りないもの |
|---|---|---|
| 5 | 加算の項数が壁になる | ラダー未完・1条件1run（実験9 段階3 で測定中） |

**「課題の規模が壁」は支持されない。**84通りが失敗し740通りが成功している。
...
| 「壁は項数と空間の2軸」 | 09/19 の記述。尺度が未検証 |

### 未解決

- 動的参照が**難しさを増す**かどうか（実験10 で測る）
...
| **9** | 09/19〜09/20 | 課題を極小にしても動的参照は学習されないか／壁は動的参照か | **逆の結果**。動的参照は100%学習され（未出399/399），静的な6項加算が失敗した | **要再現**（1条件1run） |
