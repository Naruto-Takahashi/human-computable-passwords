...
| **12** | 09/23〜 | 素朴な手順の誤判定はどれだけ起きるか（偽陰性の再現率） | — | **実施中** |
...
### 実験12: 偽陰性の再現率（09/23〜，実施中）

**問い**: 素朴な手順の誤判定はどれだけ起きるか。第一貢献の定量化である。

**母数**: 素朴な手順（5エポック・線形減衰・評価500件）で正解率40%未満と判定された
条件は **31件**ある（10アルゴリズムにまたがる）。測り直し済みは2件のみで，
うち1件が反転した（`table_add3_k10` 鍵6，10.2% → 100.0%）。

**層別して6本回す**。1つの関数で鍵を振ると「簡単な関数を選んだ」と言われるため，
族ごとに2本ずつ測り，**どこで手順が失敗するか**まで言えるようにする。

| 族 | 条件 | 5ep判定 | 衝突確率 | 既出率 |
|---|---|---|---|---|
| 静的3項 | `table_add3_k10` 鍵1 / 鍵8 | 10.6% / 10.6% | 10.0% / 10.1% | 63% |
| 実質静的 $`m=1`$ | `narrowptr_k10_m1` 鍵1 / 鍵2 | 9.8% / 13.4% | 10.0% / 10.5% | 63% |
| 動的 $`m=2`$ | `narrowptr_k10_m2` 鍵1 / 鍵2 | 9.6% / 17.4% | 10.0% / 10.5% | **0%** |

6件とも衝突確率と区別できない位置にいる。素朴な手順ならすべて「安全」と判定される。

**片側検定として設計している。**反転は決定的（モデルが実際に学習した）だが，
反転しないことは不確定である（不運な run かもしれない — 実験7）。
したがって得られるのは**偽陰性率の下限**であり，「31件中，少なくともN件が
誤判定だった」と書ける。下限で足りるので1条件1runでよい。

**信用のための錨**: `func_22_k10` は20エポックでも反転しなかった（11.4% / 23.6%）。
**予算を増やせば何でも100%になるわけではない**ことの証拠であり，測定済みで
追加費用はゼロである。

スクリプト: `experiments/batch/run_false_negative_rate.sh`（約45時間）
