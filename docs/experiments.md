...
**「段階N」が何を指すかの正式な一覧．** これまで `docs/plan.md` §3.1.2，
...
段階2〜6 の結論の多くは保留または撤回されている．**経緯は
[training_dynamics.md](training_dynamics.md)（学習予算）と段階5b・6・7 を参照．
...
> 深さラダーを「段階2」と呼び始めたことで遡って1が空いた．

### 段階2: 深さラダー（08/16〜08/18）
...
**現在**: **無効**．鍵1・鍵2で測っており，段階5b で鍵1は静的参照ですら不動の鍵だと
判明した．「差が無い」は動的参照の性質ではない．

### 段階4: 学習量スイープ（08/23〜08/24）
...
> 構造内に持つため，段階8b で主軸に据える．

### 段階5b: 静的端点の検証（09/05〜09/06）
...
段階7 でこれが再現しないことが判明した．正しくは「離陸が5エポック以内に
起きたかどうか」である．

### 段階6: 鍵の交絡（09/09〜09/10）
...
**現在**: (B) の「鍵が決める」は**撤回**（段階7）．(A) の静的 vs 動的の対比は
固定予算下の比較として**有効**だが，「動的参照は学習できない」とまでは言えない．

### 段階7: seed ばらつきの検証（09/12〜09/13）
...
**問い**: 予算を8倍（40エポック＝5000ステップ）にすれば離陸するか．

- (A) `func_22_k10` 鍵0 — 動的参照は遅いだけなのか，本当に無理なのか（本命）
- (B) `table_add3_k10` 鍵6/ds0 — 5エポックで唯一離陸しなかった静的条件．
  離陸すれば「予算が測定器だった」という診断の裏づけになる

主データは正解率ではなく**検証損失の軌跡**．1本約13時間，2本で約26時間．
スクリプト: `experiments/batch/run_budget_probe.sh`

### 段階8b: 離陸ステップ数による測定（予定）

**問い**: 難易度を「解けるか」ではなく**「離陸に何ステップかかるか」**で測れるか．

段階5 の参照範囲ラダー（$m \in \{1,2,3,5,10\}$）を，離陸する鍵と8a で分かった予算，
**3シード**で回し直す．$m=1$ は必ず離陸するので端点として機能する．

全条件が打ち切られても「$m$ がいくつを超えると測定不能になるか」は言えるため，
段階3 のように「差が無かった」で終わらない．
...
1. **評価は 500 件**（段階2）．50件では点推定が最大13ポイントずれ，結論を誤らせた
2. **偶然水準ではなく鍵ごとの基準線と比べる**（段階2）．衝突確率 $\sum p_i^2$ と最頻値
3. **基準線を最初に測る**（段階3）．後回しにすると「差が無い」のか「識別できない」のか区別できない
4. **床にあるとき「差が無い」は効果の否定にならない**（段階3）
5. **鍵を複数引いて分布で比較する**（段階5b）．鍵1〜2本での条件間比較は解釈できない
6. **1条件1runで結論を出さない**（段階7）．設定を完全に固定しても離陸は1エポックぶれる
7. **固定エポックは予算の宣言であり，測定器になる**（段階7）．難易度を測るなら収束まで回す
8. **500件評価の前に検証損失を見る**（段階6）．鍵10本で正解率を完全に順位づけ，学習中に無料で取れる
