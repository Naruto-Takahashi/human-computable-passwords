...
| 末尾 | 「次に読む」を置いて，読者を次の文書へ送る |
| 注記 | 読み飛ばすと事故になるものは GitHub のアラート（`> [!IMPORTANT]` 等）にする |
| 図 | **関係・流れ・分岐**は mermaid。**値・判定・対比**は表。迷ったら表にする |
| 数式 | `$...$` / `$$...$$` で書く。Unicode の下付き文字（`k₁`）で代用しない |

mermaid を書いたら **`make check-mermaid`** で構文を確かめる。誤ると GitHub 上では
図の代わりにエラーが表示される。図を入れすぎると描画が遅くなるので，
1ページ1枚までを目安にする。

**結果の数値は1箇所にだけ置く。** 実験の結果は
[experiment_index.md](experiment_index.md)，計画は [plan.md](plan.md)，
日々の記録は [log.md](log.md) と役割を分け，同じ数字を2箇所に書かない。
