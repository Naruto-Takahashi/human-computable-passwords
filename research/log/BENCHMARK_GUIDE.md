...
`outputs/benchmarks/{model_name}/{paradigm}/{generator_name}/run_{timestamp}/`

### 質的分析（思考ログの確認）
各テストケースの **「生の思考プロセス」** が自動保存されます．
`outputs/benchmarks/.../reasoning_logs/` を確認してください．
...
```bash
nix develop --command python3 experiments/benchmarks/runner.py \
  --provider mock \
