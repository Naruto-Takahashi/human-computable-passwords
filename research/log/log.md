...
- **実施したこと**:
  - **大規模なディレクトリ再編**: リポジトリを `src/` (コアロジック), `experiments/` (実行スクリプト), `outputs/` (集約された出力) の3層構造に刷新．`hcp` と `llm` のパッケージ化により保守性を向上．
  - **出力管理の高度化**: ベンチマーク結果を `outputs/benchmarks/<モデル>/<アルゴリズム>/<手法>/run_<日時>` の階層構造で保存するように刷新．大規模な実験データの整理と管理を容易にした．
