...
- **実施したこと**:
  ...
  - **DeepSeek-R1 (推論モデル) への最適化**:
    ...
    - **ストリーミング通信**: タイムアウトと空レスポンスを防ぐため，Ollama との通信をストリーミング受信方式に変更し，安定性を劇的に向上させた．
    - **思考プロセスの永続化**: 全てのテストケースについて，LLMの生の思考過程を個別の Markdown ファイル（`reasoning_logs/`）として自動保存する機能を実装．
  - **パースロジックの高度化**: 
    - JSON形式による構造化回答の強制と，正規表現による柔軟な抽出を組み合わせたハイブリッドパーサーを実装．
