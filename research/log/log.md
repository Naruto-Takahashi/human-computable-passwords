...
- **実施したこと**:
  ...
  - **出力管理の高度化**: ベンチマーク結果を `outputs/benchmarks/<モデル>/<アルゴリズム>/run_<日時>` の階層構造で保存するように刷新．大規模な実験データの整理と管理を容易にした．
  - **ローカルLLM環境のGPU化**: Nix環境によるCUDAリンクの問題を解消するため，ホストOS側の Ollama へ移行．RTX 2080 Ti を 100% 活用した高速推論環境を確立．
  - **DeepSeek-R1 (推論モデル) への最適化**:
    - **長考への対応**: 推論モデルの巨大な思考プロセス（Chain-of-Thought）を許容するため，`num_predict` および `num_ctx` を `16384` へ拡張．
    - **鉄壁のストリーミング通信**: タイムアウトや接続切断による「空レスポンス」を防ぐため，ストリーミング受信方式を導入．エラー発生時でも受信済みの内容を救済して保存・評価する極めて堅牢な通信ロジックを実現．
    - **思考プロセスのリアルタイム永続化**: 1問解くごとに即座に生の思考過程を個別の Markdown ファイル（`reasoning_logs/`）として自動保存する機能を実装．中断やクラッシュに対する耐性を大幅に高めた．
  - **パースロジックの高度化**: 
    - JSON形式による構造化回答の指示と，正規表現による柔軟な抽出を組み合わせたハイブリッドパーサーを実装．
    - モデルが結論を書き忘れた場合でも `<think>` タグ内から答えを救済するフォールバック機能を搭載し，パースエラー（ERR）を最小化．
