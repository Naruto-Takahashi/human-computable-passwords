#!/usr/bin/env bash
# =============================================================================
# 段階5b: 「静的参照は100%学習できる」という端点の前提を検証する
#
# 【なぜ必要になったのか】
# 段階5（参照範囲ラダー）で，静的参照の端点として設計した narrowptr_k10_m1
...
#   ・key_seed = 0 の1本のみ
#   ・評価 50 件のみ
# であり，鍵を揃えた確認をしていなかった（段階4で前例の引き違えをした
# のと同じ種類の誤り）。
#
# 【7月と今回で違う点は2つに絞られる】
...
#     学習条件は段階3・4・5と完全に同一（pure / stage2 / n_train=1000 / epochs=5）。
# =============================================================================
set -uo pipefail
...
echo "=== [$(date '+%m/%d %H:%M:%S')] 段階5b 静的端点の検証 完了 ===" >> "$LOGDIR/static_endpoint_done.log"
