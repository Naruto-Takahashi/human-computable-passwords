...
#
# 【なぜ必要になったのか】
# 段階5b（2026/09/06）で，関数もルール文も学習条件も同一のまま鍵だけを
...
#
# 段階3（構造ラダー）・段階4（学習量スイープ）・段階5（参照範囲ラダー）は
...
#     段階5b の時点では3点しかなく，「異なる数字が7種類」でも ks=0（99.6%）と
#     ks=2（26.2%）で大きく違ったため，仮説を立てるには点が足りない。
#     約14時間。
#
# 【今後の標準手順】
# 条件間比較は鍵を複数引いて分布で行う。鍵1〜2本での比較は解釈できない
# ことが段階5b で確定したため，このバッチ以降そのように運用する。
#
# 学習条件は段階3・4・5・5b と完全に同一（pure / stage2 / n_train=1000 /
# epochs=5），評価は500件。
# =============================================================================
set -uo pipefail
...
echo "=== [$(date '+%m/%d %H:%M:%S')] 段階6 鍵の交絡 完了 ===" >> "$LOGDIR/key_effect_done.log"
