...
> [!WARNING]
> **走行中は，バッチスクリプトと `src/hcp/algorithms.py` を編集しない。**
>
> - **バッチスクリプト**: bash はバイト位置で逐次読むため，行を足すと実行位置が
>   ずれて別の箇所が再実行される（2026-09-12 に実際に発生し，同じ条件を2回学習した）
> - **`algorithms.py`**: 評価は学習とは**別プロセス**で後から起動するため，
>   既存アルゴリズムの定義を変えると，学習時と評価時で出題が食い違う
>
> どちらも直したいときはバッチの終了を待つ。やむを得ず触った場合は，
> 走行中の条件の定義が変わっていないことを確認すること（バッチ起動時の
> コミットと `rule_text` / `fn` を突き合わせる）。
>
> **ドキュメント・走っていないバッチ・`tests/` は触ってよい。**実行時に読まれない。
