# 人間計算可能パスワード (HCP) に基づく LLM の帰納的ルール推論限界と暗号解読能力の評価ベンチマーク

このリポジトリは，  
**人間計算可能パスワード（Human-Computable Password，HCP）に基づく LLM の帰納的ルール推論限界と暗号解読能力の評価ベンチマーク**  
...
本研究では，HCPが持つ「人間の暗算で実行できる簡潔さ」と「間接参照（ポインタ）やモジュロ演算（剰余）などの構造的・非線形なアルゴリズム関係」という性質に着目し，**LLMにおける「帰納적ルール特定能力の限界（臨界点）」を定量的に明らかにするためのベンチマーク**として再定義しました．

予備実験において，ローカルLLM単体では入出力ペアのみから背後にあるルールを完全逆推定することはほぼ不可能であることが示されました．本ベンチマークでは，完全に不可能なブラックボックス状態から出発し，プロンプトへ提示する情報（アルゴリズム仕様や鍵の部分開示数 $K$）を段階的に変化（Stage 0〜3）させていくことで，**AIの推論が「崩壊」から「解読成功」へと転移する境界（相転移境界）**を数理的・実証的に特定・スキャンします．
...
1. **「データ汚染（Data Contamination）」からの完全な脱却**:  
   独自のアルゴリズムと乱数シードから無限に未知の入出力パターンを合成可能なため，LLMが丸暗記している懸念のない純粋な「インコンテキスト帰納推論能力」を測定できます．
2. **帰納推論が崩壊する「相転移境界（臨界点）」の特定**:  
...
4. **従来の機械学習との「データ効率性」の対比**:  
   数万のデータを用いた「教師あり学習」によって近似的に関数を再現する従来の機械学習に対し，僅かな Few-shot から論理構造を理解しようとするLLMの帰納バイアスの違いを測定します．
